...
import LiveCustomizerDeck from './components/LiveCustomizerDeck';
...
const getUnifiedButtonClass = (theme: string, isPrimary: boolean = true, brandAccent: string = '#857850') => {
  const base = "font-bold transition-all duration-300 hover:scale-[1.03] active:scale-[0.97] ease-out shadow-lg border cursor-pointer";
  ...
  if (isPrimary) {
    return theme === 'dark'
      ? `${base} bg-[var(--brand-accent-color)] text-white border-transparent hover:bg-transparent hover:text-[var(--brand-accent-color)] hover:border-[var(--brand-accent-color)] shadow-white/[0.02]`
      : `${base} bg-[var(--brand-accent-color)] text-white border-transparent hover:bg-transparent hover:text-[var(--brand-accent-color)] hover:border-[var(--brand-accent-color)] shadow-neutral-950/[0.05]`;
  } else {
    return theme === 'dark'
      ? `${base} bg-white/[0.04] text-white border-white/[0.08] hover:border-[var(--brand-accent-color)] hover:bg-[var(--brand-accent-color)]/[0.08] shadow-sm`
      : `${base} bg-[#e9e6dc] text-neutral-900 border border-[#dfdbd0] hover:bg-white hover:border-[var(--brand-accent-color)] hover:text-[var(--brand-accent-color)] shadow-sm`;
  }
};
...
export default function App() {
  ...
  // 1. BRAND PORTAL CUSTOMIZATION STATES
  const [brandName, setBrandName] = useState(() => localStorage.getItem('sylvan-brand-name') || 'Sylvan');
  const [brandIcon, setBrandIcon] = useState(() => localStorage.getItem('sylvan-brand-icon') || 'tree'); // 'tree' | 'star' | 'target' | 'flame' | 'crown'
  const [brandAccent, setBrandAccent] = useState(() => localStorage.getItem('sylvan-brand-accent') || 'dynamic'); // default dynamic contrast (white/black)
  const [brandTagline, setBrandTagline] = useState(() => localStorage.getItem('sylvan-brand-tagline') || 'Sylvan gives creators, educators, and coaches a fully branded space with courses, events, discussions, and members—all under your own command.');
  
  // 2. AMBIENT AUDIO CLICK FEEDBACK STATES
  const [soundEnabled, setSoundEnabled] = useState(() => {
    const saved = localStorage.getItem('sylvan-sound-enabled');
    return saved !== null ? saved === 'true' : true;
  });

  // Haptic audio click synthesizer using Web Audio API (completely offline-first, zero-asset latency)
  const playHapticClick = useCallback((type: 'click' | 'success' | 'toggle' = 'click') => {
    ...
    try {
      ...
      console.warn("Audio context failed or blocked by autoplay browser guidelines", error);
    }
  }, [soundEnabled]);

  // Update localStorage when customization state changes
  const saveBrandConfig = useCallback((name: string, icon: string, accent: string, tagline: string) => {
    setBrandName(name);
    setBrandIcon(icon);
    setBrandAccent(accent);
    setBrandTagline(tagline);
    localStorage.setItem('sylvan-brand-name', name);
    localStorage.setItem('sylvan-brand-icon', icon);
    localStorage.setItem('sylvan-brand-accent', accent);
    localStorage.setItem('sylvan-brand-tagline', tagline);
  }, []);

  const saveSoundSetting = useCallback((enabled: boolean) => {
    setSoundEnabled(enabled);
    localStorage.setItem('sylvan-sound-enabled', String(enabled));
  }, []);

  // 3. SOFT THEME TRANSITION VEIL
  const [isVeiling, setIsVeiling] = useState(false);
  const [veilingTheme, setVeilingTheme] = useState<'dark' | 'light'>('dark');

  // Preloader and Skeleton states
  const [showPreloader, setShowPreloader] = useState(true);
  const [isSkeletonLoading, setIsSkeletonLoading] = useState(true);

  // Desktop scroll-driven stacked card animation state & refs
  const [isDesktop, setIsDesktop] = useState(false);
  const bentoSectionRef = useRef<HTMLDivElement>(null);

  const resolvedAccentColor = brandAccent === 'dynamic'
    ? (theme === 'dark' ? '#ffffff' : '#000000')
    : brandAccent;

  useEffect(() => {
    document.documentElement.style.setProperty('--brand-accent-color', resolvedAccentColor);
  }, [resolvedAccentColor]);

  // 4. LENIS PREMIUM SMOOTH SCROLL INITIALIZATION
  useEffect(() => {
  ...
  useEffect(() => {
    const checkIsDesktop = () => {
      setIsDesktop(window.innerWidth >= 1024); // lg breakpoint is 1024px to stack normally on tablets & mobile
    };
    checkIsDesktop();
  ...
  // Local framer-motion useScroll instance scoped specifically to bentoSectionRef
  // Scoped to 140px top start so animations only run while in the centered sticky visual zone
  const { scrollYProgress } = useScroll({
    target: bentoSectionRef,
    offset: ["start 140px", "end end"]
  });

  // Smooth scroll progress using a spring for high-refresh-rate butter-smooth transitions
  const smoothProgress = useSpring(scrollYProgress, {
    stiffness: 45, // slightly less stiff for a smoother, luxurious glide
    damping: 30,   // higher damping for a more luxurious deceleration
    mass: 0.4,     // slightly higher mass for realistic inertia and momentum
    restDelta: 0.0001
  });

  // Transforms for Card 1 (always visible, scales down for depth)
  const card1Scale = useTransform(smoothProgress, [0, 0.35, 0.45, 0.70, 0.80], [1, 1, 0.94, 0.94, 0.88], { clamp: true });
  // Fade out Card 1 when Card 2 settles to eliminate overlay rendering lag and bottom peek-through
  const card1Opacity = useTransform(smoothProgress, [0, 0.35, 0.45], [1, 1, 0], { clamp: true });

  // Transforms for Card 2 (slides up, then scales down when Card 3 comes over it)
  const card2Y = useTransform(smoothProgress, [0, 0.12, 0.40, 1.0], [600, 600, 0, 0], { clamp: true });
  const card2Scale = useTransform(smoothProgress, [0, 0.50, 0.80, 1.0], [1, 1, 0.94, 0.94], { clamp: true });
  // Fade out Card 2 when Card 3 settles to eliminate rendering lag
  const card2Opacity = useTransform(smoothProgress, [0, 0.12, 0.35, 0.70, 0.80], [0, 0, 1, 1, 0], { clamp: true });

  // Transforms for Card 3 (slides up)
  const card3Y = useTransform(smoothProgress, [0, 0.50, 0.80, 1.0], [600, 600, 0, 0], { clamp: true });
  const card3Opacity = useTransform(smoothProgress, [0, 0.50, 0.75, 1.0], [0, 0, 1, 1], { clamp: true });

  // Active-card pointer events & z-index mappings for bulletproof overlay clicking on desktop
  const card1PointerEvents = useTransform(smoothProgress, (value: number) => value < 0.38 ? "auto" : "none");
  const card2PointerEvents = useTransform(smoothProgress, (value: number) => (value >= 0.38 && value < 0.73) ? "auto" : "none");
  const card3PointerEvents = useTransform(smoothProgress, (value: number) => value >= 0.73 ? "auto" : "none");

  const card1ZIndex = useTransform(smoothProgress, (value: number) => value < 0.38 ? 30 : 10);
  const card2ZIndex = useTransform(smoothProgress, (value: number) => (value >= 0.38 && value < 0.73) ? 30 : 10);
  const card3ZIndex = useTransform(smoothProgress, (value: number) => value >= 0.73 ? 30 : 10);

  useEffect(() => {
    // End skeleton loading after a realistic duration
  ...
  const [card1Empty, setCard1Empty] = useState<boolean>(false);
  const [card2Empty, setCard2Empty] = useState<boolean>(false);
  const [card3Empty, setCard3Empty] = useState<boolean>(false);
  ...
  // Page routing state
  const [currentPage, setCurrentPage] = useState<'landing' | 'behance' | 'dashboard'>('landing');
  ...
  useEffect(() => {
    ...
    const iosDevice = isIPad || isIPhone;
    setIsIOS(iosDevice);
    ...
    const handleBeforeInstallPrompt = (e: Event) => {
      ...
      // Only show banner if they haven't explicitly dismissed it this session
      const isDismissed = sessionStorage.getItem('sylvan-pwa-dismissed');
      if (!isDismissed) {
    ...
    // If already launched as PWA standalone, do not show banner
    if (window.matchMedia('(display-mode: standalone)').matches || (window.navigator as any).standalone) {
      setShowInstallBanner(false);
    } else {
      // For iOS, if they haven't dismissed it, we can show a subtle prompt after 4 seconds
      const isDismissed = sessionStorage.getItem('sylvan-pwa-dismissed');
  ...
  const handleInstallClick = async () => {
    ...
    if (!deferredPrompt) {
      alert("PWA installation is supported via your browser menu (click the 3 dots or download icon in your address bar).");
      return;
    }
  ...
  const dismissInstallBanner = () => {
    sessionStorage.setItem('sylvan-pwa-dismissed', 'true');
    setShowInstallBanner(false);
  };
  ...
  // Reset window scroll position to top instantly whenever switching pages and force scroll recalculations
  useEffect(() => {
    window.scrollTo(0, 0);
    
    // Slight timeout allows browser layout calculations to settle after display change from hidden to block
    const timer = setTimeout(() => {
      window.scrollTo(0, 0);
  ...
  const toggleTheme = useCallback(() => {
    ...
    setVeilingTheme(nextTheme);
    setIsVeiling(true);
    playHapticClick('toggle');
    
    // Smooth transition halfway through the veil sweep
  ...
  return (
    <>
      <style>{`
        :root {
          --brand-accent-color: ${resolvedAccentColor} !important;
        }
        
        /* Dynamic Theme Highlights & Accents across the full web app */
        .text-emerald-400, 
        .text-emerald-500, 
        .text-blue-500, 
        .text-blue-400, 
        ...
        /* Accent outlines and dynamic ring focus */
        .focus\\:border-\\[var\\(--brand-accent-color\\)\\]:focus,
        input:focus, 
        textarea:focus, 
        select:focus {
          border-color: var(--brand-accent-color) !important;
          box-shadow: 0 0 0 2px ${resolvedAccentColor}20 !important;
        }
      `}</style>
      {showPreloader && (
      ...
      <div 
        id="home" 
        className={`min-h-screen font-sans antialiased overflow-x-clip relative transition-colors duration-500 ${
          theme === 'dark' 
      ...
      <div className={currentPage === 'landing' ? 'block' : 'hidden'}>
          {/* HERO SECTION */}
          <div className={`relative w-full overflow-x-clip transition-colors duration-500 ${theme === 'dark' ? 'bg-[#070708]' : 'bg-[#fbfaf7]'}`}>
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden z-0 select-none opacity-90">
          {/* Main 4K fluid abstract image with elegant blur and slow GPU-accelerated drift animation */}
          <div 
            className="absolute inset-0 bg-cover bg-center blur-[50px] saturate-[1.2] animate-bg-drift transition-opacity duration-500" 
            style={{ 
          ...
          />
          {/* Subtle soft textured overlay layer with reverse drift */}
          <div 
            className="absolute inset-0 bg-cover bg-center mix-blend-overlay scale-110 blur-[8px] animate-bg-drift-reverse transition-opacity duration-500" 
            style={{ 
              backgroundImage: `url('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=2400&q=80')`,
        ...
        <div className="relative z-10 max-w-3xl mt-4">
          ...
          <motion.div 
            ...
            className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full backdrop-blur-md border text-xs font-semibold mb-6 shadow-sm transition-all duration-300 ${
              theme === 'dark' 
                ...
                : 'bg-transparent border-neutral-300 text-neutral-950'
            }`}
          >
          ...
          >
            <span className="relative block w-full">
              ...
              <span className="invisible pointer-events-none select-none block max-sm:line-clamp-3 max-sm:overflow-hidden">
                Your curriculum deserves a beautiful brand.
              </span>
              <span className="absolute inset-x-0 top-0 block text-center max-sm:line-clamp-3 max-sm:overflow-hidden">
                <Typewriter 
                  words={[
                    "Your community deserves its own home.",
                    "Your members deserve a calm space.",
                    "Your curriculum deserves a beautiful brand.",
                    "Your movement deserves its own host.",
                    "Your audience deserves a place to connect."
                  ]}
                  typingSpeed={50}
                  deletingSpeed={25}
                  pauseDuration={3000}
          ...
          >
            <button 
              ...
              className={`px-8 py-3.5 rounded-full text-sm ${getUnifiedButtonClass(theme, true, brandAccent)}`}
            >
              Get started free
            </button>
          </motion.div>
        ...
        <div className="w-full max-w-4xl mt-16 relative z-20 [perspective:2000px] select-none">
          ...
          <div className={`h-12 border-b flex items-center justify-between px-3 sm:px-4 transition-all duration-300 ${
            theme === 'dark' ? 'bg-neutral-900/60 border-neutral-900' : 'bg-neutral-100/90 border-neutral-200'
          }`}>
            {/* Window controls */}
            <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
              <div className="w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-[#ff5f56]/80" />
              <div className="w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-[#ffbd2e]/80" />
              <div className="w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-[#27c93f]/80" />
            </div>
            {/* Mock URL Bar */}
            <div className={`h-7 w-[60%] sm:w-1/2 px-2 sm:px-4 rounded-full border flex items-center justify-center gap-1 sm:gap-1.5 text-[9px] sm:text-xs select-none transition-all duration-300 ${
              theme === 'dark' ? 'bg-[#070708] border-white/[0.08] text-neutral-400' : 'bg-white border-neutral-300 text-neutral-600 shadow-sm'
            }`}>
              <Globe className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-neutral-500 shrink-0" />
              <span className="truncate max-w-full">community.sylvan-saas.vercel.app</span>
            </div>
            {/* Action spacer */}
            <div className="flex items-center gap-1.5 w-10 sm:w-12 justify-end shrink-0">
          ...
          <div className={`flex h-[450px] md:h-[480px] backdrop-blur-xl text-left transition-colors duration-300 ${
              ...
              {[
                ...
                return (
                  <button 
                    ...
                    className={`w-full flex items-center justify-center sm:justify-between px-1.5 py-2 sm:px-2.5 rounded-xl text-left text-xs font-medium outline-none focus:outline-none transition-all duration-150 border ${
                      isSelected 
                        ? theme === 'dark'
                  ...
                  >
                    ...
                    {tab.count && (
                      <span className={`w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center shrink-0 hidden sm:flex ${
                        theme === 'dark' ? 'bg-white/10 text-white' : 'bg-neutral-950/15 text-neutral-950'
                      }`}>
                        {tab.count}
                      </span>
            ...
            <div className={`flex-1 p-4 md:p-6 overflow-y-auto flex flex-col transition-all duration-300 ${
              ...
              <AnimatePresence mode="wait">
                  ...
                  >
                    ...
                    >
                      <div className="z-10 max-w-[70%]">
                        <span className={`text-[10px] font-bold uppercase tracking-wider ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-600'}`}>Featured Community</span>
                    ...
                    <div className="grid grid-cols-3 gap-1.5 sm:gap-3">
                      <div className={`backdrop-blur-sm p-2 sm:p-3 rounded-xl sm:rounded-2xl border flex flex-col justify-between transition-all ${
                        theme === 'dark' ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-neutral-100/70 border-neutral-200'
                      }`}>
                        <span className="text-[8px] sm:text-[10px] text-neutral-500 block font-medium uppercase truncate leading-none" title="Active Members">Active Members</span>
                        <span className={`text-xs sm:text-sm md:text-base font-bold mt-1 block whitespace-nowrap leading-none ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`}>1,280</span>
                      </div>
                      <div className={`backdrop-blur-sm p-2 sm:p-3 rounded-xl sm:rounded-2xl border flex flex-col justify-between transition-all ${
                        theme === 'dark' ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-neutral-100/70 border-neutral-200'
                      }`}>
                        <span className="text-[8px] sm:text-[10px] text-neutral-500 block font-medium uppercase truncate leading-none" title="Course Takers">Course Takers</span>
                        <span className={`text-xs sm:text-sm md:text-base font-bold mt-1 block whitespace-nowrap leading-none ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`}>324</span>
                      </div>
                      <div className={`backdrop-blur-sm p-2 sm:p-3 rounded-xl sm:rounded-2xl border flex flex-col justify-between transition-all ${
                        theme === 'dark' ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-neutral-100/70 border-neutral-200'
                      }`}>
                        <span className="text-[8px] sm:text-[10px] text-neutral-500 block font-medium uppercase truncate leading-none" title="Forum">Forum</span>
                        <span className={`text-xs sm:text-sm md:text-base font-bold mt-1 block whitespace-nowrap leading-none ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`}>12 Active</span>
                      </div>
                    </div>
                    ...
                    <motion.div 
                      ...
                      transition={{ type: "spring", stiffness: 400, damping: 25 }}
                      className={`backdrop-blur-sm rounded-2xl p-3 sm:p-4 mt-1 flex flex-col sm:flex-row sm:items-center justify-between gap-3 cursor-pointer border ${
                    ...
                    >
                      <div className="flex items-start gap-2.5 sm:gap-3 text-left">
                        <img 
                          ...
                          alt="Ava" 
                          className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full object-cover border shrink-0 ${theme === 'dark' ? 'border-neutral-800' : 'border-[#cbc6b8]'}`}
                          referrerPolicy="no-referrer"
                        />
                        <div className="min-w-0 flex-1">
                          <span className={`text-[11px] sm:text-xs font-semibold block leading-tight ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`}>
                            Join Ava's circle discussions
                          </span>
                          <span className={`text-[10px] block mt-1 leading-normal ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>
                            Participate in design critiques and sprints
                          </span>
                        </div>
                      </div>
                      <button 
                        ...
                        className={`w-full sm:w-auto px-4 py-1.5 sm:py-2 rounded-full text-[11px] sm:text-xs transition-all duration-200 shadow-sm shrink-0 text-center font-bold border border-transparent ${
                          brandAccent === 'dynamic'
                            ? theme === 'dark' 
                              ? 'bg-white text-neutral-950 hover:bg-neutral-100' 
                              : 'bg-neutral-950 text-white hover:bg-neutral-900'
                            : 'bg-[var(--brand-accent-color)] text-white hover:opacity-90'
                        }`}
                      >
                        Join now
                      </button>
                ...
                {activeHeroTab === 'Chat' && (
                  ...
                  >
                    {/* Header */}
                    <div className={`pb-2 flex items-center justify-between gap-2 shrink-0 border-b ${
                      ...
                      <div className="min-w-0 flex-1 text-left">
                        ...
                        <p className={`text-[10px] mt-0.5 truncate ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>Casual design conversations and feedback loops.</p>
                      </div>
                      <span className={`text-[10px] flex items-center gap-1.5 shrink-0 whitespace-nowrap ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>
                      ...
                      <div className={`pb-3 flex flex-col gap-2.5 border-b ${
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right flex flex-col items-end">
                            <span className={`text-[11px] font-mono font-bold ${theme === 'dark' ? 'text-white' : 'text-neutral-950'}`}>{progressPercent}%</span>
                          ...
                          return (
                            <div key={module.id} className={`backdrop-blur-sm rounded-2xl overflow-hidden border transition-colors ${
                              ...
                              <button 
                                ...
                                className={`w-full px-4 py-3 flex items-center justify-between gap-2 text-left text-xs font-medium transition-colors ${
                              ...
                              >
                                <div className="flex items-center gap-2 min-w-0 flex-1">
                                  ...
                                  <span className={`font-semibold text-[11px] truncate whitespace-nowrap ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`} title={module.title}>
                                    {module.title}
                                  </span>
                                </div>
                                <div className={`flex items-center gap-1.5 shrink-0 ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>
                                  <span className="text-[9px] font-mono whitespace-nowrap">{module.lessons.length} lessons</span>
                              ...
                              <AnimatePresence>
                                {isExpanded && (
                                  ...
                                  >
                                    <div className="px-4 pt-1.5 pb-4 flex flex-col gap-2.5 text-left w-full">
                                      {module.lessons.map((lesson, idx) => {
                                        ...
                                        return (
                                          <div 
                                            ...
                                            className={`flex items-center gap-2.5 py-1.5 text-[11px] cursor-pointer transition-colors group select-none min-w-0 w-full ${
                                            ...
                                            <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-all ${
                                              isCompleted
                                                ...
                                                : theme === 'dark'
                                                  ...
                                                  : 'border-[#dfdbd0] bg-neutral-50 text-transparent group-hover:border-neutral-400'
                                            }`}>
                                              <Check className="w-3 h-3 stroke-[3.5px]" />
                                            </div>
                                            <span 
                                              className={`transition-all duration-300 truncate whitespace-nowrap flex-1 min-w-0 ${isCompleted ? 'line-through text-neutral-500 font-medium' : ''}`}
                                              title={lesson}
                                            >
                                              {lesson}
                                            </span>
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-2.5">
                      {[
                        ...
                        return (
                          <div key={ev.id} className={`backdrop-blur-sm rounded-2xl p-4 flex flex-col gap-3.5 border transition-all duration-300 ${
                            theme === 'dark' ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-neutral-100/70 border-neutral-200'
                          }`}>
                            <div className="text-left">
                              <span className={`text-[10px] font-bold uppercase tracking-wide ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-600'}`}>{ev.date} • {ev.time}</span>
                              <h4 className={`text-xs font-semibold mt-1 leading-snug ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`}>{ev.title}</h4>
                              <span className={`text-[10px] block mt-1 ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>Hosted by {ev.host}</span>
                            </div>
                            <button 
                              onClick={() => toggleHeroRSVP(ev.id)}
                              className={`w-full py-2 rounded-xl text-[10.5px] font-bold transition-all duration-150 shrink-0 text-center ${
                                isRSVPed 
                                  ? theme === 'dark' 
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                      {LEADERBOARD_MEMBERS.map(member => (
                        <div key={member.name} className={`backdrop-blur-sm p-2.5 sm:p-3 rounded-2xl flex items-center gap-2.5 border transition-colors ${
                          theme === 'dark' ? 'bg-white/[0.03] border-white/[0.06]' : 'bg-[#f8f6f0]/85 border-[#dfdbd0]'
                        }`}>
                          ...
                          <div className="overflow-hidden min-w-0 flex-1 text-left">
                            <span className={`text-xs font-medium block truncate ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`} title={member.name}>{member.name}</span>
                            <span className={`text-[9px] block uppercase font-medium truncate ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>{member.role}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                        ...
                        <div key={member.rank} className={`backdrop-blur-sm px-3.5 py-2.5 rounded-2xl flex items-center justify-between border transition-colors ${
                          ...
                          <div className="flex items-center gap-3 min-w-0 flex-1">
                            ...
                            <img 
                              ...
                              referrerPolicy="no-referrer"
                            />
                            <span className={`text-xs font-medium truncate whitespace-nowrap min-w-0 ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`}>{member.name}</span>
                          </div>
                          <span className={`text-[11px] font-mono shrink-0 hidden sm:inline-block ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-650'}`}>{member.points} pts</span>
                        </div>
                      ))}
                    </div>
                  </motion.div>
      ...
      <section id="features" className="py-24 px-6 max-w-7xl mx-auto scroll-mt-20 relative">
        ...
        {/* Tab Selection Row - stacked vertically on mobile, row on sm and up, highly touch-friendly */}
        <div className="flex flex-col sm:flex-row flex-wrap justify-center items-stretch sm:items-center gap-3 sm:gap-6 md:gap-8 my-10 max-w-2xl mx-auto w-full px-4 sm:px-0">
          {coreFeatureTabs.map(tab => {
            ...
            return (
              <button 
                ...
                className={`px-6 py-3.5 sm:py-2.5 rounded-full text-sm font-semibold tracking-wide transition-all duration-300 shrink-0 border hover:scale-[1.02] sm:hover:scale-[1.04] hover:shadow-lg hover:shadow-neutral-500/5 text-center cursor-pointer ${
                  isSelected 
                    ? brandAccent === 'dynamic'
                      ? theme === 'dark'
                      ...
                      : 'bg-[var(--brand-accent-color)] text-white border-[var(--brand-accent-color)] shadow-md font-bold'
                    : theme === 'dark'
          ...
          <div className={`relative z-10 w-full max-w-4xl rounded-2xl border backdrop-blur-md shadow-3xl overflow-hidden flex flex-row h-[460px] transition-all duration-300 ${
            ...
            {isSkeletonLoading ? (
              <div className="flex flex-row w-full h-full animate-pulse">
                {/* Skeleton Sidebar */}
                <div className={`w-12 md:w-56 shrink-0 border-r pt-5 pb-3 px-3 md:p-4 flex flex-col gap-4 md:gap-5 text-left h-full transition-colors duration-300 ${
                  theme === 'dark' ? 'bg-white/[0.01] border-white/[0.08]' : 'bg-[#f4f2eb]/60 border-neutral-300'
                }`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 w-full">
                      <div className={`w-5 h-5 rounded ${theme === 'dark' ? 'bg-white/10' : 'bg-neutral-900/10'}`} />
                      <div className={`h-3 w-16 rounded hidden md:block ${theme === 'dark' ? 'bg-white/10' : 'bg-neutral-900/10'}`} />
                    </div>
                  </div>
                  <div className="flex flex-col gap-2.5 mt-2">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-lg">
                        <div className={`w-3.5 h-3.5 rounded-full shrink-0 ${theme === 'dark' ? 'bg-white/10' : 'bg-neutral-900/10'}`} />
                        <div className={`h-3 w-20 rounded hidden md:block ${theme === 'dark' ? 'bg-white/10' : 'bg-neutral-900/10'}`} />
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className={`flex-1 flex flex-col h-full p-5 justify-between ${theme === 'dark' ? 'bg-white/[0.01]' : 'bg-white/40'}`}>
                  <div className="space-y-4">
                    {/* Header skeleton */}
                    <div className="flex justify-between items-center pb-4 border-b border-white/[0.04]">
                      <div className={`h-4 w-32 rounded ${theme === 'dark' ? 'bg-white/10' : 'bg-neutral-900/10'}`} />
                      <div className={`h-7 w-16 rounded-full ${theme === 'dark' ? 'bg-white/10' : 'bg-neutral-900/10'}`} />
                    </div>
                    {/* Content list skeleton */}
                    <div className="space-y-3 pt-2">
                      {[1, 2, 3].map((i) => (
                        <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border ${theme === 'dark' ? 'border-white/[0.05] bg-white/[0.02]' : 'border-neutral-200 bg-white/50'}`}>
                          <div className={`w-8 h-8 rounded-full shrink-0 ${theme === 'dark' ? 'bg-white/10' : 'bg-neutral-900/10'}`} />
                          <div className="flex-1 space-y-2">
                            <div className={`h-3 w-24 rounded ${theme === 'dark' ? 'bg-white/10' : 'bg-neutral-900/10'}`} />
                            <div className={`h-2.5 w-full rounded ${theme === 'dark' ? 'bg-white/5' : 'bg-neutral-900/5'}`} />
                          </div>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div className={`h-10 w-full rounded-xl border ${theme === 'dark' ? 'border-white/[0.05] bg-white/[0.02]' : 'border-neutral-200 bg-white/50'}`} />
                </div>
              </div>
            ) : (
              <>
                {/* Left Sidebar */}
                <div className={`w-12 md:w-56 shrink-0 border-r pt-5 pb-1.5 px-1.5 md:p-4 flex flex-col gap-3 md:gap-5 text-left h-full select-none overflow-y-auto no-scrollbar transition-all duration-300 ${
                  theme === 'dark' ? 'bg-white/[0.01] border-white/[0.08]' : 'bg-[#f4f2eb]/60 border-neutral-300'
                }`}>
                  {/* Header Info */}
                  <div className="flex items-center justify-center md:justify-between">
                    <div className="flex items-center gap-1.5">
                      <Leaf className={`w-3.5 h-3.5 shrink-0 transition-colors duration-300 ${theme === 'dark' ? 'text-white' : 'text-neutral-950'}`} strokeWidth={2.5} />
                        ...
                        {isCoreTyping && (
                          ...
                          >
                            <img 
                              ...
                              alt="Typing indicator member avatar"
                              className="w-7 h-7 rounded-full object-cover border border-white/[0.08] shrink-0" 
                              referrerPolicy="no-referrer" 
                            />
                    ...
                    >
                      ...
                      <div className={`h-14 border-b px-5 flex items-center justify-between shrink-0 select-none transition-colors duration-300 ${theme === 'dark' ? 'border-white/[0.08]' : 'border-[#dfdbd0]'}`}>
                        ...
                        <div className="flex flex-col items-end sm:flex-row sm:items-center gap-1 sm:gap-3">
                          <span className={`text-[10.5px] font-bold ${theme === 'dark' ? 'text-white' : 'text-neutral-950'}`}>
                        ...
                        {[
                          ...
                          return (
                            <div 
                              ...
                              className={`p-3 sm:p-3.5 rounded-xl border flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 transition-all cursor-pointer group active:scale-[0.99] select-none ${
                                isActive 
                                  ? theme === 'dark'
                                  ...
                                  : isCompleted
                                    ...
                                    : theme === 'dark'
                                      ...
                                      : 'bg-neutral-50/60 border-[#dfdbd0]'
                              }`}
                            >
                                ...
                                {isCompleted ? (
                                  <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 border ${
                                    theme === 'dark'
                                      ...
                                      : 'bg-neutral-950/10 border border-neutral-300 text-neutral-950'
                                  }`}>
                                    <Check className="w-3 h-3" />
                                  </div>
                                ) : isActive ? (
                                  <div className={`w-5 h-5 rounded-full flex items-center justify-center group-hover:scale-105 transition-transform shrink-0 ${
                                    theme === 'dark'
                                      ? 'bg-white/10 border border-white/35 text-white'
                                      : 'bg-neutral-950/10 border border-[#dfdbd0] text-neutral-950'
                                  }`}>
                                  ...
                                  <div className={`w-5 h-5 rounded-full flex items-center justify-center border transition-colors shrink-0 ${
                                    theme === 'dark' ? 'bg-white/[0.02] border-white/[0.08] text-neutral-600' : 'bg-neutral-100 border-[#dfdbd0] text-neutral-400'
                                  }`}>
                                    <Lock className="w-2.5 h-2.5" />
                                  </div>
                                )}
                                <span className={`text-[11.5px] font-semibold text-left transition-colors truncate whitespace-nowrap ${
                                  isLocked 
                                    ? 'text-neutral-500' 
                              ...
                              <div className="flex items-center gap-2 pl-8 sm:pl-0 shrink-0">
                                ...
                                {isCompleted && (
                                  <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded tracking-wide whitespace-nowrap ${
                                    theme === 'dark' ? 'text-neutral-400 bg-white/5' : 'text-neutral-600 bg-neutral-950/5'
                                  }`}>Completed</span>
                                )}
                              </div>
                            </div>
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-3 no-scrollbar">
                      {[
                        ...
                        <div key={idx} className={`p-4 rounded-xl border flex flex-col gap-3.5 text-left transition-all duration-300 ${
                          theme === 'dark' 
                            ? 'bg-white/[0.03] border-white/[0.08] backdrop-blur-sm' 
                            : 'bg-white/85 border-[#dfdbd0] shadow-sm'
                        }`}>
                          <div className="flex flex-col gap-1 w-full">
                            <span className={`text-[9px] font-bold uppercase tracking-wide transition-colors duration-300 ${
                              theme === 'dark' ? 'text-white/80' : 'text-neutral-900/80'
                            }`}>{ev.date}</span>
                            <span className={`text-xs sm:text-[11.5px] font-semibold leading-snug transition-colors duration-300 ${theme === 'dark' ? 'text-white' : 'text-neutral-950'}`}>{ev.title}</span>
                            <div className="flex items-center gap-1.5 mt-1">
                              <img src={ev.host} alt="Host avatar" className="w-4 h-4 rounded-full object-cover border border-white/5" referrerPolicy="no-referrer" />
                              <span className={`text-[10px] transition-colors duration-300 ${theme === 'dark' ? 'text-neutral-500' : 'text-[#8c897d]'}`}>Hosted by Creator • {ev.rsvps} attending</span>
                            </div>
                          </div>
                          <button 
                            onClick={() => {
                              if (idx === 0) {
                                setCoreEventsRSVP(!coreEventsRSVP);
                              }
                            }}
                            className={`w-full py-2 rounded-xl text-[10px] font-bold text-center transition-all hover:scale-[1.01] active:scale-[0.98] duration-200 shrink-0 ${
                              ev.active 
                                ? theme === 'dark' 
                                  ? 'bg-white text-neutral-950 hover:bg-neutral-200 shadow-sm' 
                                  : 'bg-neutral-950 text-white hover:bg-neutral-900 shadow-sm'
                      ...
                      {[
                        ...
                        <div key={idx} className={`flex items-center justify-between border p-3 rounded-xl transition-all duration-300 ${
                          ...
                          <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            <span className={`text-[10px] font-bold shrink-0 ${idx === 0 ? 'text-yellow-500' : 'text-neutral-500'}`}>{member.rank}</span>
                            <img src={member.avatar} alt={`${member.name}'s avatar`} className="w-6.5 h-6.5 rounded-full object-cover border border-white/[0.08] shrink-0" referrerPolicy="no-referrer" />
                            <div className="text-left min-w-0 flex-1">
                              <div className="flex items-center gap-1.5 flex-wrap sm:block">
                                <span className={`text-xs font-bold transition-colors duration-300 truncate block ${theme === 'dark' ? 'text-white' : 'text-neutral-900'}`}>{member.name}</span>
                                <span className={`text-[9px] font-bold uppercase transition-colors duration-300 truncate sm:block ${theme === 'dark' ? 'text-neutral-400' : 'text-[#8c897d]'}`}>
                                  <span className="sm:hidden normal-case text-neutral-400 mr-1.5">•</span>
                                  {member.tag}
                                </span>
                              </div>
                            </div>
                          </div>
                          <span className={`hidden sm:inline-block text-[11px] font-mono font-bold px-2.5 py-0.5 rounded-full border transition-all duration-300 shrink-0 ${
                            theme === 'dark'
      ...
      <section 
        ref={bentoSectionRef} 
        className={`px-6 relative transition-colors duration-300 bg-transparent z-20 ${
          isDesktop ? 'pt-32 pb-16 h-[320vh] overflow-visible' : 'py-24 overflow-hidden'
        }`}
      >
        {/* Inject mathematically centered and header-clamped responsive top offsets for the sticky cards including their sticky section header */}
        <style dangerouslySetInnerHTML={{ __html: `
          @media (min-width: 1024px) {
            .bento-sticky-container {
              position: sticky !important;
              top: max(88px, calc(50vh - 370px)) !important;
            }
          }
          @media (min-width: 1280px) {
            .bento-sticky-container {
              position: sticky !important;
              top: max(88px, calc(50vh - 400px)) !important;
            }
          }
        ` }} />

        {/* Sticky container ONLY for bento cards to allow 100% stable center locking */}
        <div className={
          isDesktop 
            ? "bento-sticky-container w-full max-w-7xl mx-auto h-auto flex flex-col justify-start overflow-visible pt-2 pb-6 z-20"
            : "max-w-7xl mx-auto w-full relative z-20"
        }>
          {/* Section Header is now placed INSIDE the sticky container so it stays stuck and perfectly visible as cards overlay */}
          <div className="w-full relative z-20 mb-10 md:mb-12 lg:mb-14">
            ...
            <motion.div 
              ...
              viewport={{ once: true, margin: "-100px" }}
              transition={{ duration: 0.6, ease: "easeOut" }}
              className={`grid md:grid-cols-12 gap-6 md:gap-12 items-end`}
            >
              <div className="md:col-span-7 text-center md:text-left flex flex-col items-center md:items-start">
                ...
                <h2 className={`text-3xl lg:text-4xl xl:text-5xl font-bold mt-3 lg:mt-5 tracking-tight leading-none transition-colors duration-300 ${
                  theme === 'dark' ? 'text-white' : 'text-neutral-900'
                }`}>
                  Set up once.
                </h2>
                <h2 className={`text-3xl lg:text-4xl xl:text-5xl font-bold mt-1 lg:mt-2 tracking-tight leading-none transition-colors duration-300 ${
                  theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'
                }`}>
                  Run it the way you want.
                </h2>
              </div>
              <div className="md:col-span-5 text-center md:text-left md:pb-1">
                <p className={`text-xs lg:text-sm xl:text-base leading-relaxed font-normal transition-colors duration-300 min-h-[50px] ${
          ...
          <div 
            className={isDesktop ? "relative w-full h-[460px] lg:h-[500px] xl:h-[540px] overflow-visible" : "mt-16 flex flex-col gap-10 sm:gap-14 lg:gap-16 relative w-full"}
            style={{ willChange: 'transform' }}
          >
            
            {/* CARD 1: YOUR FRONT DOOR */}
            <div className={isDesktop ? "absolute inset-0" : "w-full"}>
              <motion.div 
                initial={isDesktop ? false : { opacity: 0, y: 50 }}
                whileInView={isDesktop ? undefined : { opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-100px" }}
                transition={{ duration: 0.7, ease: "easeOut" }}
                whileHover={isDesktop ? undefined : { scale: 1.01 }}
                style={
                  isDesktop 
                    ? { 
                ...
                className={`w-full relative overflow-hidden rounded-3xl border flex flex-col md:flex-row h-auto md:h-[460px] lg:h-[500px] xl:h-[540px] transition-colors duration-300 origin-top ${
                  theme === 'dark' 
              ...
              >
                {/* Left side info pane */}
                <div className={`md:w-5/12 p-6 lg:p-8 flex flex-col justify-between text-left h-full border-b md:border-b-0 md:border-r transition-all duration-300 ${
                  theme === 'dark' 
                  ...
                  <div>
                    ...
                    <h3 className={`text-xl md:text-2xl lg:text-3xl font-bold mt-6 md:mt-4 lg:mt-6 leading-tight tracking-tight transition-colors duration-300 ${
                      theme === 'dark' ? 'text-white' : 'text-neutral-900'
                    }`}>
                      A community overview page that sells itself.
                    </h3>
                    <p className={`text-xs sm:text-sm mt-4 md:mt-3 lg:mt-4 leading-relaxed font-normal transition-colors duration-300 ${
                      theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'
                    }`}>
                  ...
                  {/* Bottom tag */}
                  <div className={`mt-6 md:mt-0 flex items-center gap-2 text-xs font-medium transition-colors duration-300 ${
                    theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'
                  }`}>
                ...
                <div className={`md:w-7/12 relative overflow-hidden flex flex-col h-full min-h-[440px] md:min-h-0 justify-center transition-colors duration-300 ${
                  ...
                  <div className="absolute top-4 left-4 right-4 z-20 flex items-center justify-between select-none">
                    <div className="flex items-center gap-1.5">
                      ...
                      <button 
                        onClick={() => setCard1Empty(!card1Empty)}
                        title={card1Empty ? "Populate Demo Data" : "Clear Data (Empty State)"}
                        className={`px-2.5 h-8 rounded-lg border backdrop-blur-md flex items-center justify-center gap-1 text-[10px] font-bold transition-all ${
                          card1Empty 
                            ? 'bg-[#c58d5c]/30 border-[#c58d5c]/50 text-amber-100' 
                            : 'bg-black/45 border-white/[0.06] text-neutral-300 hover:text-white'
                        }`}
                      >
                        <span className={`w-1.5 h-1.5 rounded-full ${card1Empty ? 'bg-amber-400 animate-pulse' : 'bg-emerald-400'}`} />
                        {card1Empty ? "Empty State" : "Demo Data"}
                      </button>
                    </div>
                  ...
                  <div className={`relative z-10 w-[90%] max-w-[340px] backdrop-blur-lg border p-6 rounded-2xl flex flex-col items-center shadow-2xl text-center select-none mx-auto my-14 md:my-0 transition-all duration-300 ${
                    ...
                    {card1Empty ? (
                      <div className="py-4 flex flex-col items-center w-full">
                        <div className={`w-12 h-12 rounded-full border-2 border-dashed flex items-center justify-center mb-3 transition-colors ${
                          theme === 'dark' ? 'border-white/[0.12] text-neutral-400' : 'border-neutral-300 text-neutral-500'
                        }`}>
                          <Leaf className="w-5 h-5 opacity-40" strokeWidth={1.5} />
                        </div>
                        <h4 className={`text-sm font-bold tracking-tight transition-colors ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-800'}`}>Awaiting Your Title</h4>
                        <p className={`text-[11px] leading-relaxed mt-2 max-w-[210px] transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>
                          No branding metadata has been saved. Populate details to construct your community's elegant front sanctuary.
                        </p>
                        <div className={`mt-4 text-[10px] font-mono font-medium tracking-wider uppercase px-2.5 py-0.5 rounded-full border transition-colors ${
                          theme === 'dark' ? 'bg-white/[0.02] border-white/[0.06] text-neutral-500' : 'bg-neutral-100 border-neutral-200 text-neutral-500'
                        }`}>
                          0 Members Connected
                        </div>
                      </div>
                      ...
                      <>
                        ...
                        <a 
                          href="https://sylvan-saas.vercel.app/"
                          target="_blank"
                          rel="noopener noreferrer"
                          className={`flex items-center gap-1.5 mt-0.5 cursor-pointer transition-colors ${
                            theme === 'dark' ? 'text-neutral-400 hover:text-white' : 'text-neutral-500 hover:text-neutral-900'
                          }`}
                        >
                          <span className="text-xs">sylvan-saas.vercel.app</span>
                        ...
                        <div className={`flex items-center gap-2 mt-3 px-3 py-1 rounded-full border transition-all duration-300 ${
                          ...
                          <div className="flex -space-x-1.5">
                            <img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=40&h=40&q=80" alt="Community member avatar" className="w-4.5 h-4.5 rounded-full object-cover border border-black" referrerPolicy="no-referrer" />
                            <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=40&h=40&q=80" alt="Community member avatar" className="w-4.5 h-4.5 rounded-full object-cover border border-black" referrerPolicy="no-referrer" />
                            <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=40&h=40&q=80" alt="Community member avatar" className="w-4.5 h-4.5 rounded-full object-cover border border-black" referrerPolicy="no-referrer" />
                          </div>
                        ...
                        <button 
                          onClick={() => handleOpenJoinModal()} 
                          className={`w-full py-3 rounded-xl text-xs mt-4 ${getUnifiedButtonClass(theme, true, brandAccent)}`}
                        >
                          Join now
                        </button>
                      </>
            ...
            <div className={isDesktop ? "absolute inset-0" : "w-full"}>
              <motion.div 
                ...
                className={`w-full relative overflow-hidden rounded-3xl border flex flex-col-reverse md:flex-row h-auto md:h-[460px] lg:h-[500px] xl:h-[540px] transition-colors duration-300 origin-top ${
                  theme === 'dark' 
              ...
              >
                  ...
                  <div className={`relative z-10 w-[92%] max-w-[420px] backdrop-blur-xl border p-5 rounded-2xl flex flex-col shadow-2xl select-none mx-auto my-8 md:my-0 transition-all duration-300 ${
                    ...
                    {card2Empty ? (
                      <div className="py-12 px-4 flex flex-col items-center justify-center text-center">
                        ...
                        <h6 className={`text-sm font-bold tracking-tight transition-colors ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-800'}`}>Standings Undetermined</h6>
                        <p className={`text-[11px] leading-relaxed mt-2 max-w-[240px] transition-colors ${
                          theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'
                        }`}>
                          No actions recorded in this window. Engage in community posts and complete tasks to claim Sylvan's top standings.
                        </p>
                      </div>
                    ) : (
                      <>
                        ...
                        <>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg transition-colors ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 420 pts</p>
                            </div>
                          </div>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg transition-colors ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 210 pts</p>
                            </div>
                          </div>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg border transition-all duration-300 ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 64 pts</p>
                            </div>
                          </div>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg transition-colors ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 88 pts</p>
                            </div>
                          </div>
                        </>
                      )}
                        ...
                        <>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg transition-colors ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 1,450 pts</p>
                            </div>
                          </div>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg transition-colors ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 1,120 pts</p>
                            </div>
                          </div>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg border transition-all duration-300 ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 650 pts</p>
                            </div>
                          </div>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg transition-colors ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 340 pts</p>
                            </div>
                          </div>
                        </>
                      )}
                        ...
                        <>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg border transition-all duration-300 ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 4,200 pts</p>
                            </div>
                          </div>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg transition-colors ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 3,900 pts</p>
                            </div>
                          </div>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg transition-colors ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 2,800 pts</p>
                            </div>
                          </div>
                          ...
                          <div className={`grid grid-cols-12 gap-2 items-center py-1.5 px-1 rounded-lg transition-colors ${
                            ...
                            <div className="col-span-4 text-right">
                              ...
                              <p className={`text-[9px] font-medium transition-colors ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>+ 1,100 pts</p>
                            </div>
                          </div>
                        </>
                      )}
                ...
                <div className={`md:w-5/12 p-6 lg:p-8 flex flex-col justify-between text-left h-full border-b md:border-b-0 transition-all duration-300 ${
                  theme === 'dark' 
              ...
              >
                ...
                <div className={`md:w-7/12 relative overflow-hidden flex flex-col h-full min-h-[440px] md:min-h-0 justify-center transition-colors duration-300 ${
                  ...
                  <div className={`relative z-10 w-[94%] max-w-[460px] backdrop-blur-xl border rounded-2xl flex flex-col shadow-2xl overflow-hidden select-none mx-auto my-8 md:my-0 transition-all duration-300 ${
                    ...
                    <div className={`px-2 py-2 sm:px-4 sm:py-3 border-b flex items-center justify-between gap-1 transition-colors ${
                      ...
                      <div className="flex items-center gap-1 sm:gap-1.5 text-left shrink-0">
                        ...
                        <button className={`w-6 h-6 sm:w-7 sm:h-7 rounded flex items-center justify-center border transition-colors ${
                          theme === 'dark' 
                        ...
                        >
                          <span className="hidden xs:inline">Chapter 1</span>
                          <span className="xs:hidden">Ch 1</span>
                        </button>
                        <button 
                        ...
                        >
                          <span className="hidden xs:inline">Chapter 2</span>
                          <span className="xs:hidden">Ch 2</span>
                        </button>
                        
                        <button className={`hidden sm:flex w-6 h-6 rounded items-center justify-center transition-all text-xs font-bold border ${
                          theme === 'dark'
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className={`px-2 py-1 rounded border flex items-center gap-1 text-[10px] font-bold cursor-pointer transition-colors ${
                          theme === 'dark' 
                            ...
                            : 'bg-neutral-950/10 border-neutral-300 text-neutral-950'
                        }`}>
                          <span>Live</span>
                    ...
                    <div className="p-5 flex flex-col gap-4 text-left relative min-h-[320px] justify-center">
                      {card3Empty ? (
                        <div className="py-8 px-4 flex flex-col items-center justify-center text-center w-full">
                          <div className={`w-12 h-12 rounded-full border-2 border-dashed flex items-center justify-center mb-3 transition-colors ${
                            theme === 'dark' ? 'border-white/[0.12] text-neutral-400' : 'border-neutral-300 text-neutral-500'
                          }`}>
                            <Sliders className="w-5 h-5 opacity-40" />
                          </div>
                          <h6 className={`text-sm font-bold tracking-tight transition-colors ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-800'}`}>Classroom Untouched</h6>
                          <p className={`text-[11px] leading-relaxed mt-2 max-w-[250px] transition-colors ${
                            theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'
                          }`}>
                            No syllabus chapters or lessons have been saved. Populate syllabus modules to begin designing Sylvan's customized course maps.
                          </p>
                        </div>
                      ) : (
                        <>
                      ...
                      {courseChapter === 'ch-1' ? (
                        <div className="mt-8 flex flex-col gap-3">
                          ...
                          <div className={`text-[11px] leading-relaxed font-normal transition-colors ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-700'}`}>
                            What you'll need is to leave with a <br className="sm:hidden" /><span className={`inline-block px-1.5 py-0.5 rounded-full border font-semibold ${
                              theme === 'dark' ? 'border-white/[0.08] bg-white/[0.06] text-white' : 'border-[#dfdbd0] bg-[#e9e6dc] text-neutral-900'
                        ...
                        <div className="mt-8 flex flex-col gap-3">
                          ...
                          <div className={`text-[11px] leading-relaxed font-normal transition-colors ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-700'}`}>
                            Learn to conduct high-impact user research interviews that extract <br />
                            <span className={`inline-block whitespace-nowrap px-1.5 py-0.5 rounded-full border font-semibold ${
                          ...
                          <div className={`border-l-2 p-2.5 rounded-r-lg text-[10px] italic leading-normal transition-colors ${
                            theme === 'dark' 
                              ? 'border-white/40 bg-white/[0.02] text-neutral-400' 
                              : 'border-neutral-400 bg-neutral-100/50 text-neutral-600'
                          }`}>
      ...
      <section id="pricing" className="py-24 px-6 max-w-7xl mx-auto scroll-mt-20 relative">
        ...
        <motion.div 
          ...
          viewport={{ once: true, margin: "-100px" }}
          transition={{ duration: 0.6, ease: "easeOut" }}
          className="grid md:grid-cols-12 gap-10 md:gap-6 items-start md:items-end justify-between mb-16"
        >
          <div className="md:col-span-7 text-center md:text-left flex flex-col items-center md:items-start">
            <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-[11px] font-semibold transition-colors duration-300 ${
              theme === 'dark' 
        ...
        <motion.div 
          ...
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
          className="grid md:grid-cols-3 gap-12 md:gap-6 items-stretch mt-12 md:py-8"
        >
              ...
              >
                <div>
                  ...
                  <button 
                    ...
                    className={`w-full py-3 rounded-full text-xs mt-6 ${getUnifiedButtonClass(theme, isPro, brandAccent)}`}
                  >
                    {plan.buttonText}
                  </button>
      ...
      <section id="faq" className="py-24 px-6 max-w-7xl mx-auto scroll-mt-20 relative overflow-hidden transition-colors duration-300">
        ...
        <motion.div 
          ...
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
          className="grid md:grid-cols-12 gap-12 md:gap-8 items-start"
        >
          {/* Left Side: Categories and Support Card */}
          <div className="md:col-span-4 flex flex-col gap-10 md:gap-6">
            {/* Category tabs */}
            <motion.div 
          ...
          <motion.div 
            variants={faqCardVariants}
            className="md:col-span-8 flex flex-col gap-6 md:gap-4"
          >
            <AnimatePresence mode="wait">
              <motion.div 
      ...
      <section id="blog" className="py-28 px-6 max-w-7xl mx-auto scroll-mt-20 relative overflow-hidden transition-colors duration-300">
        ...
        <motion.div 
          ...
          whileInView="visible"
          viewport={{ once: true, margin: "-100px" }}
          className="grid grid-cols-1 md:grid-cols-3 gap-10 sm:gap-8 md:gap-6"
        >
          {isSkeletonLoading ? (
            [1, 2, 3].map((i) => (
              <div 
                key={i}
                className={`rounded-[24px] p-4 pb-6 flex flex-col justify-between border animate-pulse ${
                  theme === 'dark' ? 'bg-[#0b0b0d] border-white/[0.06]' : 'bg-[#fdfdfc] border-[#dfdbd0]'
                }`}
              >
                <div>
                  {/* Aspect ratio identical to original */}
                  <div className={`aspect-[16/10] rounded-[16px] ${
                    theme === 'dark' ? 'bg-white/5' : 'bg-neutral-200'
                  }`} />
                  <div className="mt-5 space-y-3">
                    <div className={`h-5 w-5/6 rounded-md ${
                      theme === 'dark' ? 'bg-white/10' : 'bg-neutral-200'
                    }`} />
                    <div className={`h-5 w-2/3 rounded-md ${
                      theme === 'dark' ? 'bg-white/10' : 'bg-neutral-200'
                    }`} />
                  </div>
                </div>
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-12 gap-12 items-center relative z-10">
          ...
          >
            ...
            <p className={`text-sm sm:text-base mt-6 leading-relaxed max-w-sm mx-auto md:mx-0 text-center md:text-left font-normal transition-colors duration-300 min-h-[80px] ${
              ...
              <Typewriter 
                words={["Join Sylvan beta for free. Create your space, invite your first members, and bring everything together—under your name."]}
                typingSpeed={25}
                showCursor={false}
                oneWay={true}
                triggerInView={true}
            ...
            <button 
              onClick={() => handleOpenJoinModal()}
              className={`px-8 py-3.5 text-sm rounded-full mt-8 ${getUnifiedButtonClass(theme, true, brandAccent)}`}
            >
              Start for free
            </button>
          </motion.div>
          ...
          >
            <div className={`w-full lg:max-w-[1020px] md:max-w-[820px] max-w-[540px] rounded-[24px] overflow-hidden flex lg:h-[640px] md:h-[540px] h-[440px] relative z-10 transition-all duration-300 border ${
              ...
              {/* Mockup Sidebar */}
              <div className={`w-12 sm:w-[30%] border-r pt-5 pb-3 px-3 sm:p-5 lg:p-7 flex flex-col gap-4 sm:gap-6 lg:gap-8 shrink-0 text-left transition-colors duration-300 ${
                ...
                <div className="flex items-center justify-center sm:justify-between w-full">
                  <div className="flex items-center gap-1.5 shrink-0">
                    <Leaf className={`w-3.5 h-3.5 shrink-0 ${theme === 'dark' ? 'text-white' : 'text-neutral-950'}`} strokeWidth={2.5} />
                    <span className={`font-sans font-bold text-xs tracking-tight hidden sm:inline ${theme === 'dark' ? 'text-white' : 'text-neutral-950'}`}>Sylvan</span>
                  </div>
              ...
              <div className={`flex-1 p-4 sm:p-6 lg:p-12 flex flex-col items-center justify-center text-center relative overflow-hidden transition-colors duration-300 ${
                ...
                <div className="flex items-center justify-center gap-2 mt-2 sm:mt-3">
                  <div className="flex -space-x-2">
                    <img src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=48&h=48&q=80" alt="Community member avatar" className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border object-cover ${theme === 'dark' ? 'border-neutral-900' : 'border-neutral-100'}`} referrerPolicy="no-referrer" />
                    <img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=48&h=48&q=80" alt="Community member avatar" className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border object-cover ${theme === 'dark' ? 'border-neutral-900' : 'border-neutral-100'}`} referrerPolicy="no-referrer" />
                    <img src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=48&h=48&q=80" alt="Community member avatar" className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border object-cover ${theme === 'dark' ? 'border-neutral-900' : 'border-neutral-100'}`} referrerPolicy="no-referrer" />
                  </div>
                ...
                <button 
                  onClick={() => handleOpenJoinModal()}
                  className={`w-full max-w-[200px] sm:max-w-[240px] lg:max-w-[280px] py-2.5 sm:py-3 lg:py-3.5 rounded-full text-[11px] sm:text-xs lg:text-sm font-extrabold mt-4 sm:mt-6 lg:mt-8 transition-all hover:scale-105 active:scale-95 hover:shadow-xl shadow-lg border border-transparent ${
                    brandAccent === 'dynamic'
                      ? theme === 'dark' 
                ...
                {registrationSuccess ? (
                  ...
                  >
                    <div className={`w-14 h-14 rounded-full border flex items-center justify-center mb-5 transition-colors duration-300 ${
                      theme === 'dark' ? 'bg-white/10 border-white/20 text-white' : 'bg-neutral-950/15 border-neutral-300 text-neutral-950'
                    }`}>
                  ...
                  <form onSubmit={handleRegister} className="flex flex-col gap-5">
                    ...
                    <button 
                      type="submit"
                      className={`w-full py-4 rounded-xl text-xs mt-2 ${getUnifiedButtonClass(theme, true, brandAccent)}`}
                    >
      ...
      <AnimatePresence>
        {showInstallBanner && (
          <motion.div 
            ...
            className={`fixed bottom-6 right-6 left-6 md:left-auto md:w-[380px] z-[45] border rounded-2xl p-5 shadow-2xl flex flex-col gap-4 transition-colors duration-300 ${
              theme === 'dark' 
                ? 'bg-[#0c0c0e] border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.8)]' 
                : 'bg-[#fcfbf9] border-[#dfdbd0] shadow-[0_15px_35px_rgba(140,137,125,0.12)]'
            }`}
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl border flex items-center justify-center shrink-0 transition-colors duration-300 ${
                  theme === 'dark' ? 'bg-white/[0.04] border-white/[0.08] text-white' : 'bg-neutral-100 border-neutral-350 text-neutral-900'
                }`}>
                ...
                <div className="text-left">
                  <h4 className={`text-xs font-bold tracking-tight transition-colors duration-300 ${
                    theme === 'dark' ? 'text-white' : 'text-neutral-950'
                  }`}>Install Sylvan App</h4>
                  <p className={`text-[10px] mt-0.5 leading-relaxed transition-colors duration-300 ${
                    theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'
                  }`}>Save resources offline for seamless high-speed access.</p>
                </div>
              </div>
              <button 
                onClick={dismissInstallBanner}
                className={`p-1 rounded-full transition-all active:scale-95 ${
                  theme === 'dark' ? 'hover:bg-white/[0.06] text-neutral-400' : 'hover:bg-neutral-200/60 text-neutral-600'
                }`}
              >
            ...
            <div className="flex items-center gap-2">
              <button 
                onClick={dismissInstallBanner}
                className={`flex-1 py-2 rounded-lg text-[11px] font-semibold transition-colors border text-center ${
                  theme === 'dark' 
                    ? 'border-white/[0.06] hover:bg-white/[0.04] text-neutral-400' 
              ...
              <button 
                ...
                className={`flex-1 py-2 rounded-lg text-[11px] font-semibold transition-all text-center flex items-center justify-center gap-1.5 ${
                  theme === 'dark' 
        ...
        {showIOSTip && (
          ...
          >
            <motion.div 
              ...
              onClick={(e) => e.stopPropagation()}
              className={`border rounded-3xl w-full max-w-sm p-6 shadow-2xl relative transition-all duration-300 text-center ${
                theme === 'dark' ? 'bg-[#0c0c0e] border-white/[0.08]' : 'bg-[#fcfbf9] border-[#dfdbd0]'
              }`}
            >
              <div className="flex flex-col items-center">
                <div className={`w-12 h-12 rounded-full border flex items-center justify-center mb-4 transition-colors duration-300 ${
                  theme === 'dark' ? 'bg-white/[0.04] border-white/[0.08] text-white' : 'bg-neutral-100 border-[#dfdbd0] text-neutral-950'
                }`}>
                  <Smartphone className="w-5 h-5 text-current" />
                </div>
                ...
                <p className={`text-xs mt-2 leading-relaxed max-w-[280px] transition-colors duration-300 ${
                  theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'
                }`}>
                  Install Sylvan on your iOS device directly through Safari using these quick steps:
                </p>

                <div className="w-full flex flex-col gap-3 mt-5 text-left">
                  <div className={`p-3 rounded-xl border flex items-center gap-3 transition-colors duration-300 ${
                    theme === 'dark' ? 'bg-white/[0.02] border-white/[0.06]' : 'bg-neutral-100/50 border-neutral-200'
                  }`}>
                    <div className="w-6 h-6 rounded-full bg-neutral-500/20 flex items-center justify-center text-[11px] font-bold shrink-0">1</div>
                    <p className={`text-[11px] leading-snug transition-colors duration-300 ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-700'}`}>
                      Tap the <span className="font-semibold">Share</span> button in Safari's bottom toolbar.
                    </p>
                  ...
                  <div className={`p-3 rounded-xl border flex items-center gap-3 transition-colors duration-300 ${
                    ...
                    <div className="w-6 h-6 rounded-full bg-neutral-500/20 flex items-center justify-center text-[11px] font-bold shrink-0">2</div>
                    <p className={`text-[11px] leading-snug transition-colors duration-300 ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-700'}`}>
                      Scroll down and tap <span className="font-semibold">"Add to Home Screen"</span>.
                    </p>
                    <div className="w-5 h-5 rounded border border-neutral-400/50 flex items-center justify-center text-[10px] font-bold shrink-0">+</div>
                  </div>
                </div>

                <button 
                  onClick={() => setShowIOSTip(false)}
                  className={`w-full py-3 rounded-xl text-xs mt-6 font-semibold transition-all ${
                    theme === 'dark' 
                      ? 'bg-white text-black hover:bg-neutral-200' 
                      : 'bg-neutral-950 text-white hover:bg-neutral-850'
                  }`}
      ...
      <LiveCustomizerDeck 
        ...
        onSoundToggle={saveSoundSetting}
        playHapticClick={playHapticClick}
        theme={theme}
      />

      {/* SOFT THEME TRANSITION VEIL */}
      <AnimatePresence>
        {isVeiling && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            className={`fixed inset-0 z-[10000] pointer-events-none flex items-center justify-center ${
              veilingTheme === 'dark' ? 'bg-[#070708]' : 'bg-[#fbfaf7]'
            }`}
          >
            <div className="flex flex-col items-center gap-4">
              <motion.div 
                ...
                className={`w-10 h-10 rounded-full border-2 border-t-transparent ${
                  veilingTheme === 'dark' ? 'border-white' : 'border-neutral-900'
                }`}
              />
              <span className={`text-xs font-mono tracking-widest uppercase transition-colors duration-300 ${
                veilingTheme === 'dark' ? 'text-white/60' : 'text-neutral-900/60'
              }`}>
                Adjusting Ambient Tone...
              </span>
            </div>
          </motion.div>
        )}
